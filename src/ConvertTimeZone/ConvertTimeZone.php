...
class ConvertTimeZone {
    ...
    public static function getList(){
        return array_flip(static::$timezone);
    }

    //this function converts string from $fromTimezone to $toTimeZone
    public static function convert($string , $fromTimezone , $toTimeZone , $format = 'Y-m-d H:i:s'){

        if(empty($string)){
            throw new \Exception('String need convert invalid.');
        }

        if (!in_array($fromTimezone, array_values(static::$timezone))){
            throw new \Exception('from timezone invalid.');
        }

        if (!in_array($toTimeZone, array_values(static::$timezone))){
            throw new \Exception('to timezone invalid.');
        }

        $new_str = new DateTime($string, new DateTimeZone($fromTimezone) );
        $new_str->setTimeZone(new DateTimeZone( $toTimeZone ));
